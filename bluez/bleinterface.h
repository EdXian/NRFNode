...
public:
    ...
    QLowEnergyService* get_m_service() const
    {
        return (QLowEnergyService*)m_service;
    }
    void list_Charateristics(void){
        //QList<QLowEnergyCharacteristic> characteristics() const;
        QList<QLowEnergyCharacteristic> ch_list = m_service->characteristics();

        for(int i=0;i< ch_list.length();i++){
            qDebug()<<ch_list[i].uuid().toString();
        }
...
signals:
    ...
    void dataReceived(const QPair<QLowEnergyCharacteristic, QByteArray> &data);
